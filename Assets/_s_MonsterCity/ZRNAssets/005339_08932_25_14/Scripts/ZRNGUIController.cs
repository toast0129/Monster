...
public class ZRNGUIController : MonoBehaviour {
	...
	void Start () {
		...
		this.GetComponent<CameraController_ZR>().ChangeCamera(0);
		...
		if (menuVisible == true)
		{
			...
			if (operateCameraNumber < 100)
			{
				...
				if (GUI.Button (new Rect (400, 60, 50, 40), "<---"))
				{
					...
					{
						operateCameraNumber = this.GetComponent<CameraController_ZR>().targetCameraNames.Count -1;
				...
				{
					operateCameraNumber++;
					if (operateCameraNumber > this.GetComponent<CameraController_ZR>().targetCameraNames.Count -1)
					{
						operateCameraNumber = 0;
						previousCameraNumber = operateCameraNumber;
					}
				}
				GUI.Label (new Rect (460, 60, 140, 20), this.GetComponent<CameraController_ZR>().targetCameraNames[operateCameraNumber]);
				if (GUI.Button (new Rect (450, 80, 150, 20), "Change"))
				{
					this.GetComponent<CameraController_ZR>().ChangeCamera(operateCameraNumber);
					previousCameraNumber = operateCameraNumber;
					SetQueryChan(0);
				}
			}
